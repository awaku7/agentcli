...
def test_handle_mcp_v2_requires_tool_name() -> None:
    ...
    assert "tool_name" in out.lower() or "required" in out.lower() or "error" in out.lower()
...
def test_handle_mcp_v2_server_name_not_found(repo_tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from uagent.tools.handle_mcp_v2_tool import run_tool

    cfg = repo_tmp_path / "mcp_servers.json"
    cfg.write_text(json.dumps({"mcp_servers": [{"name": "other", "url": "http://example.com/mcp"}]}), encoding="utf-8")
...
def test_handle_mcp_v2_uses_http_and_truncates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cb = ToolCallbacks(truncate_output=lambda tool, text, limit: f"TRUNC({tool})[{text}]")
    monkeypatch.setattr(m, "get_callbacks", lambda: cb)

    out = m.run_tool({"url": "http://example.com", "tool_name": "some_tool", "tool_arguments": {"a": 1}})
...
def test_format_result_saves_returned_file_payload(repo_tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    import uagent.tools.handle_mcp_v2_tool as m
