...
class TestFragmentAssemblyBuffer:
    ...
    def test_single_fragment_message(self) -> None:
        ...
        header = FragmentHeader(
            fragment_id=_fid(1), fragment_index=0, total_fragments=1, message_type=2
        )
    ...
    def test_two_fragment_assembly(self) -> None:
        ...
        sender = b"\xaa" * 8
        h1 = FragmentHeader(
            fragment_id=_fid(1), fragment_index=0, total_fragments=2, message_type=2
        )
        h2 = FragmentHeader(
            fragment_id=_fid(1), fragment_index=1, total_fragments=2, message_type=2
        )
    ...
    def test_multiple_transfers_independent(self) -> None:
        ...
        h2_0 = FragmentHeader(
            fragment_id=_fid(2), fragment_index=0, total_fragments=1, message_type=2
        )
    ...
    def test_expiry_removes_incomplete(self) -> None:
        ...
        sender = b"\xaa" * 8
        h = FragmentHeader(
            fragment_id=_fid(99), fragment_index=0, total_fragments=2, message_type=2
        )
    ...
    def test_max_inflight_limit(self) -> None:
        ...
        for i in range(2):
            h0 = FragmentHeader(
                fragment_id=_fid(i), fragment_index=0, total_fragments=2, message_type=2
            )
            buf.append(sender, h0, b"first_")
        # Third transfer should be rejected
        h = FragmentHeader(
            fragment_id=_fid(99), fragment_index=0, total_fragments=1, message_type=2
        )
    ...
    def test_parse_fragment_payload(self) -> None:
        ...
        payload = (
            fid + (0).to_bytes(2, "big") + (3).to_bytes(2, "big") + bytes([5]) + b"data"
        )
