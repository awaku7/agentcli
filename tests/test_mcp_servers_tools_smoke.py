...
def test_mcp_servers_init_template_smoke(repo_tmp_path: Path) -> None:
    ...
    obj = json.loads(out)
    assert obj["ok"] is True
    assert obj["action"] == "init_template"
...
def test_mcp_servers_add_list_validate_set_default_remove_smoke(
    ...
    out_set_default = run_tool(
        {"action": "set_default", "server_name": "third", "path": str(path)}
    )
    set_default_obj = json.loads(out_set_default)
    assert set_default_obj["ok"] is True
    ...
    remove_obj = json.loads(out_remove)
    assert remove_obj["ok"] is True
