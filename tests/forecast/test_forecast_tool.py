...
def test_parse_dates_iso():
    ...
    df = pd.DataFrame(
        {"dt": ["2024-01-01T00:00:00", "2024-01-02T12:30:00"], "v": [1, 2]}
    )
    sys.path.insert(0, str(Path(__file__).parents[2] / "src"))
    from uagent.tools.forecast_tool import _parse_date_column
...
def test_select_best_model_rmse(sample_df):
    ...
    with patch("uagent.tools.forecast_tool._get_available_models") as mock_get:
        ...
        dummy_model.predict.return_value = valid["value"].values + np.random.normal(
            0, 0.1, len(valid)
        )
...
def test_i18n_ja(monkeypatch):
    ...
    assert (
        data["ja"]["error.data_too_small"]
        == "データが不十分です: 少なくとも %(min_rows)d 行が必要ですが、%(actual)d 行を取得しました"
    )
