...
from uagent.runtime import legacy_zai_round
...
def test_zai_round_preserves_reasoning_message_and_result_contract(monkeypatch) -> None:
    core = _Core()
    kwargs = _kwargs(core)
    monkeypatch.setattr(
        legacy_zai_round,
    ...
    result = legacy_zai_round.run_legacy_zai_round(**kwargs)
...
def test_zai_round_failure_returns_without_logging(monkeypatch) -> None:
    core = _Core()
    emitted = []
    kwargs = _kwargs(core)
...
def test_zai_round_executes_tool_calls_before_continuing(monkeypatch) -> None:
    core = _Core()
    kwargs = _kwargs(core)
    tool_call = {
        "id": "call-1",
