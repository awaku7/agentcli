...
def test_path_nonexistent(repo_tmp_path: Path) -> None:
    ...
    r = _run({"path": str(repo_tmp_path / "nope.txt"), "replacement": "x", "preview": False})
    assert r["ok"] is False
...
def test_negative_occurrence(repo_tmp_path: Path) -> None:
    ...
    f.write_text("a\na\na\n", encoding="utf-8")
    r = _run({"path": str(f), "pattern": "a", "replacement": "b",
              "occurrence": -1, "preview": False})
...
def test_negative_line_no(repo_tmp_path: Path) -> None:
    ...
    f.write_text("a\nb\n", encoding="utf-8")
    r = _run({"path": str(f), "action": "insert_at_line", "line_no": -1,
              "replacement": "x\n", "preview": False})
    assert "ok" in r
...
def test_line_no_exceeds_file(repo_tmp_path: Path) -> None:
    ...
    f.write_text("a\n", encoding="utf-8")
    r = _run({"path": str(f), "action": "insert_at_line", "line_no": 9999,
              "replacement": "x\n", "preview": False})
    assert "ok" in r
...
def test_empty_pattern_literal(repo_tmp_path: Path) -> None:
    ...
    f.write_text("hello\n", encoding="utf-8")
    r = _run({"path": str(f), "pattern": "", "replacement": "x",
              "mode": "literal", "preview": False})
    assert "ok" in r
...
def test_unclosed_group_regex(repo_tmp_path: Path) -> None:
    ...
    f.write_text("hello\n", encoding="utf-8")
    r = _run({"path": str(f), "pattern": "(?P<name", "replacement": "x",
              "mode": "regex", "preview": False})
    assert r["ok"] is False
...
def test_invalid_encoding(repo_tmp_path: Path) -> None:
    ...
    f.write_text("hello\n", encoding="utf-8")
    r = _run({"path": str(f), "replacement": "x",
              "encoding": "nonexistent-encoding", "preview": False})
    assert r["ok"] is False
...
def test_empty_replacement(repo_tmp_path: Path) -> None:
    ...
    r = _run({"path": str(f), "pattern": "delete_me", "replacement": "",
              "preview": False})
    assert r["ok"] is True
...
def test_append_to_nonexistent_file(repo_tmp_path: Path) -> None:
    ...
    r = _run({"path": str(f), "action": "append",
              "replacement": "new\n", "preview": False})
...
def test_insert_before_empty_anchor(repo_tmp_path: Path) -> None:
    ...
    f.write_text("hello\n", encoding="utf-8")
    r = _run({"path": str(f), "action": "insert_before",
              "anchor_before": "", "replacement": "x\n", "preview": False})
    assert "ok" in r
...
def test_replace_between_no_anchor_after(repo_tmp_path: Path) -> None:
    ...
    f.write_text("start\nmiddle\nend\n", encoding="utf-8")
    r = _run({"path": str(f), "action": "replace_between",
              "anchor_before": "start", "replacement": "replaced\n",
              "preview": False})
    assert "ok" in r
...
def test_insert_at_line_zero(repo_tmp_path: Path) -> None:
    ...
    f.write_text("a\nb\n", encoding="utf-8")
    r = _run({"path": str(f), "action": "insert_at_line", "line_no": 0,
              "replacement": "header\n", "preview": False})
    assert r["ok"] is True
    content = f.read_text(encoding="utf-8")
...
def test_return_hashes(repo_tmp_path: Path) -> None:
    ...
    r = _run({"path": str(f), "pattern": "original", "replacement": "modified",
              "return_hashes": True, "preview": False})
    assert r["ok"] is True
...
def test_preview_not_blocked_by_confirm_over(repo_tmp_path: Path) -> None:
    ...
    r = _run({"path": str(f), "pattern": "line", "replacement": "ROW",
              "preview": True, "confirm_over": 1})
    assert r["ok"] is True
...
def test_expand_newline_tokens_false(repo_tmp_path: Path) -> None:
    ...
    f.write_text("hello\\nworld\n", encoding="utf-8")
    r = _run({"path": str(f), "pattern": "\\n", "replacement": " ",
              "expand_newline_tokens": False, "preview": False})
    assert r["ok"] is True
    content = f.read_text(encoding="utf-8")
...
def test_regex_with_line_anchors(repo_tmp_path: Path) -> None:
    ...
    f.write_text("aaa\nbbb\naaa\n", encoding="utf-8")
    r = _run({"path": str(f), "pattern": "^aaa$", "replacement": "AAA",
...
def test_glob_replace_all(repo_tmp_path: Path) -> None:
    ...
    f1.write_text("x\n", encoding="utf-8")
    f2.write_text("x\n", encoding="utf-8")
    r = _run({"path": str(repo_tmp_path / "sub"), "pattern": "x",
              "replacement": "y", "action": "replace_all_in_files",
              "glob": "*.txt", "preview": False})
    assert r["ok"] is True
...
def test_empty_file_replace(repo_tmp_path: Path) -> None:
    ...
    r = _run({"path": str(f), "pattern": "x", "replacement": "y",
              "preview": False})
    assert r["ok"] is True
