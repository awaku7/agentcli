...
def test_python_compile_py_compile_targets(repo_tmp_path: Path) -> None:
...
def test_replace_in_file_literal_replacement_newlines(repo_tmp_path: Path) -> None:
    from uagent.tools.replace_in_file_tool import run_tool

    p = repo_tmp_path / "b.txt"
    p.write_text("hello world\n", encoding="utf-8")
    ...
    assert p.read_text(encoding="utf-8") == "hello line1\nline2\n"
