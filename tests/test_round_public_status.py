...
def test_noninteractive_exit_code_maps_public_status():
    assert _exit_code_for_round_outcome(SimpleNamespace(_last_round_outcome={"status": "completed"})) == 0
    assert _exit_code_for_round_outcome(SimpleNamespace(_last_round_outcome={"status": "failed"})) == 1
    assert _exit_code_for_round_outcome(SimpleNamespace(_last_round_outcome={"status": "interrupted"})) == 130
