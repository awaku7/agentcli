...
def format_docs_list(items: Iterable[DocItem]) -> str:
    ...
    lines.append(f"  uag docs                 # {_('list')}")
    lines.append(f"  uag docs <name>          # {_('show')}")
    lines.append(f"  uag docs --path <name>   # {_('show path')}")
    lines.append(f"  uag docs --open <name>   # {_('open with OS')}")
    return "\n".join(lines)
