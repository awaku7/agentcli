...
def _translate_api_key(provider: str, *env_names: str) -> str:
    specific = _env_first(*env_names)
    if specific:
        return specific
    return get_provider_api_key(provider) or _env_first(
        "UAGENT_TRANSLATE_API_KEY", "UAGENT_API_KEY"
    )
