...
- ツールのメタデータ（OpenAI function schema の `TOOL_SPEC`）を `UAGENT_LANG` に応じて切り替えられるようにする
...
Agent Skills 系のツールでは、`error.*` を検証エラー、`confirm.*` を参照ファイル読み出しや破壊的操作の確認に使うと整理しやすいです。
...
- `UAGENT_LANG=ja` / `UAGENT_LANG=en` を切り替えて `TOOL_SPEC` が期待通りになることを確認する
