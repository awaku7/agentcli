...
1. `mode="section"` で必要な定義ブロックだけ取得する
...
1. パーサ分岐が肥大化する
1. `TOOL_SPEC` 説明が曖昧になり LLM が誤用しやすい
1. COBOL 既存パターンに副作用が出る
1. 他 `*2idx` の「1言語1ツール」方針と矛盾する
...
1. 最後のエントリは EOF まで
1. 子メンバーも「次の兄弟 − 1」、最後は親の end_line
