"""Compatibility rendering for normalized Inception stream events.

The Inception provider adapter emits only provider-neutral ``StreamEvent``
values.  This module keeps the historical ``parse_inception_stream`` display
behavior at the compatibility boundary without making the provider parser
aware of CLI, GUI, or Web hosts.
"""
...
def collect_inception_stream_events(
    ...
    *,
    ...
    Rendering is intentionally downstream of the provider adapter. CLI, GUI,
    and Web can replace this collector independently while the provider
    adapter continues to expose the same event contract.
    """
