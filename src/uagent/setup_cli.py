...
PROVIDER_FIELDS: dict[str, list[tuple[str, bool, str]]] = {
    ...
    "ollama": [
        ...
        ("UAGENT_OLLAMA_API_KEY", False, _("Ollama API key (optional, default: dummy)")),
        ...
        ("UAGENT_OLLAMA_REPEAT_PENALTY", False, _("Ollama repeat_penalty (default: 1.1)")),
...
def _env_lines_from_state(st: _WizardState) -> list[str]:
    ...
    for prov, label in PROVIDERS:
        section(_("%(label)s (UAGENT_PROVIDER=%(prov)s)") % {"label": label, "prov": prov})
        ...
        if stage == 2:
            if st.provider in RESPONSES_PROVIDERS:
                ...
                if st.responses_enabled:
                    ...
                    if r == "__back__":
                        continue
                    st.reasoning = [_('minimal'), _('low'), _('medium'), _('high'), _('auto')][
                        int(r) - 1
                    ]
