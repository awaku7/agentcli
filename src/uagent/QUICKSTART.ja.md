...
- `UAGENT_SHRINK_KEEP_LAST`（既定: `20`）
  - 要約後に末尾へ残す件数です。
- `UAGENT_SHRINK_CHUNK_SIZE`（既定: `50`）
  - 1回のLLM要約あたりのメッセージ数です。
  - 値が大きいほどLLM呼び出し回数が減りますが、コンテキスト長制限エラーのリスクが高まります。
  - コンテキスト長超過時は自動的にチャンクサイズを半減してリトライします。
- `UAGENT_SHRINK_SINGLE_SHOT`（既定: 未設定）
  - `1`/`true`/`yes` を設定すると、全メッセージを1回のLLM呼び出しで要約します（最速パス）。
  - コンテキスト長超過時は自動的にチャンク分割モードにフォールバックします。
