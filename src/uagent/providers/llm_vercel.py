...
def _parse_vercel_response(resp: Any) -> tuple[str, str, list[dict[str, Any]]]:
    ...
    for tc in raw_tool_calls:
        ...
        fn_args = getattr(fn_obj, "arguments", None) if hasattr(fn_obj, "arguments") else None
...
def _parse_vercel_stream(
    ...
    *,
    ...
    tool_calls_list = [v for _, v in sorted(tool_calls_acc.items()) if v["function"]["name"]]
...
def build_vercel_chat_kwargs(
    *,
    ...
    temp_env = (env_get(f"{_ENV_PREFIX}_TEMPERATURE") or env_get("UAGENT_TEMPERATURE") or "").strip()
    try:
    ...
    top_p_env = (env_get(f"{_ENV_PREFIX}_TOP_P") or env_get("UAGENT_TOP_P") or "").strip()
    if top_p_env:
        try:
    ...
    if reasoning:
        effort_used = _resolve_vercel_effort(reasoning, depname, auto_user_text=auto_user_text)
        if effort_used:
...
def vercel_chat_with_tools(
    ...
    *,
    ...
    _auto_user_text = _extract_latest_user_text(call_messages) if _reasoning_raw == "auto" else ""

    while True:
        try:
            chat_kwargs, _effort_used = build_vercel_chat_kwargs(
                depname=depname, call_messages=call_messages,
                send_tools=send_tools_this_round, req_tools=req_tools,
                reasoning=_reasoning_raw, auto_user_text=_auto_user_text,
            )
            if _effort_used:
                _label = f"LLM:auto->{_effort_used}" if _reasoning_raw == "auto" else f"LLM:{_effort_used}"
                try:
            ...
            if stream:
                assistant_text, reasoning_content, tool_calls_list = call_maybe_thread_fn(
                    lambda: _parse_vercel_stream(
                        ...
                        print_delta_fn=(
                            None if bool(getattr(core, "_is_web", False))
                            else (getattr(core, "print_stream_delta", None) or (lambda s: print(s, end="", flush=True) if s else None))
                        ),
                        core=core,
                    )
                )
            else:
                resp = call_maybe_thread_fn(lambda: client.chat.completions.create(**chat_kwargs))
                assistant_text, reasoning_content, tool_calls_list = _parse_vercel_response(resp)
            ...
            attempt_429, new_client, action = _rate_limit_retry_step(
                exception=e, provider="vercel", model=depname,
                attempt=attempt_429, max_retries=max_retries_429,
                base=retry_base, cap=retry_cap,
            ...
            if action == "retry":
                ...
                continue
            if action == "give_up":
                print(f"[{_LABEL} Error] " + _("429 retry limit (%(max_retries)s) reached.") % {"max_retries": max_retries_429})
            ...
            if BadRequestError is not None and isinstance(e, BadRequestError):
                ...
                if "does not support tools" in err_text_lower:
                    print(f"[{_LABEL} Error] " + _("Model does not support tools. Auto-disabling tools and retrying..."))
            ...
            print(f"[{_LABEL} Error] " + _("An error occurred while generating a response."))
            _maybe_print_certifi_where(e)
            print(repr(e))
            return False, client, "", "", []
