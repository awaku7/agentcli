...
def run_tool(args: Dict[str, Any]) -> str:
    ...
    if not isinstance(relative_path, str) or not relative_path.strip():
        return _(
            "err.relative_path_required",
            default="[tool error] relative_path must be a non-empty string",
        )
    if not isinstance(max_bytes, int) or max_bytes <= 0:
        return _(
            "err.max_bytes_positive",
            default="[tool error] max_bytes must be a positive integer",
        )
