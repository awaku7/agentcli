...
TOOL_SPEC = {
    ...
    "function": {
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "path": {"type": "string", "description": _("param.path.description", default="Path to the Makefile.")},
                "mode": {
                    "type": "string",
                    "enum": ["index", "section"],
                    "description": _("param.mode.description", default='"index" returns a numbered table of contents; "section" returns one selected block.'),
                },
                "section": {
                    "type": "integer",
                    "description": _("param.section.description", default="1-based section number, used only when mode='section'."),
                },
            },
            "required": ["path", "mode"],
...
_CONDITIONAL_RE = re.compile(r"^\s*(ifn?def|ifn?eq|else|endif|override|private)\b(.*)$", re.IGNORECASE)
    ...
    for start, end, text in logical:
        ...
        if define_start:
            if _END_DEFINE_RE.match(text):
                entries.append({"type": "define", "name": define_start[1], "start_line": define_start[0], "end_line": end, "label": f"define {define_start[1]}"})
                define_start = None
            continue
        ...
        if m and not text.lstrip().startswith(("::", "http:")):
            ...
            if targets and not all(t.startswith(".") for t in targets):
                entries.append({"type": "target", "name": " ".join(targets), "start_line": start, "end_line": end, "label": f"target {' '.join(targets)}"})
            continue
        m = _VAR_RE.match(text)
        if m:
            entries.append({"type": "variable", "name": m.group(1), "start_line": start, "end_line": end, "label": f"variable {m.group(1)}"})
            continue
        m = _INCLUDE_RE.match(text)
        if m:
            entries.append({"type": "include", "name": m.group(1).strip(), "start_line": start, "end_line": end, "label": f"include {m.group(1).strip()}"})
            continue
        m = _CONDITIONAL_RE.match(text)
        if m:
            entries.append({"type": "directive", "name": (m.group(1) + " " + m.group(2)).strip(), "start_line": start, "end_line": end, "label": f"directive {(m.group(1) + ' ' + m.group(2)).strip()}"})
...
def run_tool(args: dict[str, Any]) -> str:
    ...
    if not os.path.isfile(resolved):
        return _("err.file_not_found", default="Error: File not found: {path}").format(path=path)
    try:
        ...
        if mode == "index":
            ...
            return "\n".join(f"[{i}] {e['label']} (lines {e['start_line']}-{e['end_line']})" for i, e in enumerate(entries, 1))
        if mode != "section":
            return _("err.invalid_mode", default="Error: mode must be 'index' or 'section'.")
        try:
            section = int(args.get("section", 0))
        except (TypeError, ValueError):
            section = 0
        if section < 1 or section > len(entries):
            return _("err.invalid_section", default="Error: section must be between 1 and {count}.").format(count=len(entries))
