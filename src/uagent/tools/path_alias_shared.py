...
def _url_store_path() -> Path:
    return Path(
        os.environ.get("UAGENT_URL_ALIAS_FILE", "~/.uag/url_aliases.json")
    ).expanduser()
...
def save_url_aliases(aliases: dict[int, str]) -> None:
    ...
    with _STORE_LOCK:
        fd, tmp_name = tempfile.mkstemp(
            prefix="url_aliases.", suffix=".tmp", dir=str(path.parent)
        )
        try:
...
def resolve_url_alias(value: str) -> str:
    ...
    if base is None:
        raise ValueError(
            _("alias.unknown_url", default=f"unknown URL alias: @B{{{slot}}}")
        )
    ...
    if suffix and not suffix.startswith(("/", "?", "#")):
        raise ValueError(
            _(
                "alias.url_suffix",
                default="URL alias suffix must start with '/', '?' or '#'",
            )
        )
...
def _store_path() -> Path:
    return Path(
        os.environ.get("UAGENT_PATH_ALIAS_FILE", "~/.uag/path_aliases.json")
    ).expanduser()
...
def save_aliases(aliases: dict[int, Path]) -> None:
    ...
    with _STORE_LOCK:
        fd, tmp_name = tempfile.mkstemp(
            prefix="path_aliases.", suffix=".tmp", dir=str(path.parent)
        )
        try:
...
def resolve_alias(value: str) -> str:
    ...
    if root is None:
        raise ValueError(
            _("alias.unknown_path", default=f"unknown path alias: @A{{{slot}}}")
        )
...
def resolve_tool_aliases(value: Any, field_name: str | None = None) -> Any:
    ...
    if isinstance(value, list):
        if field_name in _PATH_FIELD_NAMES:
            return [
                resolve_alias(item) if isinstance(item, str) else item for item in value
            ]
        if field_name in _URL_FIELD_NAMES:
            return [
                resolve_url_alias(item) if isinstance(item, str) else item
                for item in value
            ]
        return [resolve_tool_aliases(item) for item in value]
    if isinstance(value, dict):
        return {
            key: resolve_tool_aliases(item, str(key)) for key, item in value.items()
        }
    return value
