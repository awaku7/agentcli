...
BUSY_LABEL = True
STATUS_LABEL = "tool:echonet_node_status"
...
def _object_epcs_for_eoj(eoj: str | None) -> list[int]:
    normalized = _normalize_eoj(eoj)
    if not normalized or normalized == "0EF001":
        return [0xD5, 0xD6, 0xD7, 0x80, 0x81, 0x82, 0x83, 0x8A, 0x8C, 0x9D, 0x9E, 0x9F]
    return [0x80, 0x81, 0x82, 0x83, 0x8A, 0x8C, 0x9D, 0x9E, 0x9F]
...
def _query_node(
    *,
    ...
    try:
        ...
        request_epcs = _object_epcs_for_eoj(target_eoj_text)
...
def _build_status_payload(
    *,
    ...
    objects, properties_all = _build_object_payload(target_eoj, frames)
    merged_properties = _merge_properties(properties_all)

    manufacturer = None
    model = None
    node_profile_props = {}
    if target_eoj.upper() == "0EF001":
    ...
    node = {
        ...
        "node_profile": {
            "eoj": target_eoj,
            "properties": merged_properties if target_eoj.upper() == "0EF001" else [],
        },
...
def _format_text(payload: dict[str, Any]) -> str:
    ...
    if node.get("manufacturer"):
        lines.append(f"Manufacturer: {node.get('manufacturer')}")
    if node.get("model"):
        lines.append(f"Model: {node.get('model')}")
...
def run_tool(args: dict[str, Any]) -> str:
    ...
    try:
        ...
        cache_set("node_status", cache_key, result)
        if output_format == "text":
            return _format_text(result)
        return json.dumps(result, ensure_ascii=False)
