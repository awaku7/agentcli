...
TOOL_SPEC = {
    ...
    "function": {
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "path": {"type": "string", "description": _("param.path.description", default="Path to the Swift (.swift) file.")},
...
_PATTERNS = [
    ...
    (r"^\s*" + _MOD + r"func\s+(\w+)\s*\([^)]*\)\s*(?:->\s*(?:\w+(?:<[^>]*>)?(?:\s*\|\s*\w+(?:<[^>]*>)?)*\s*)?)?(?:\{|$)",
     lambda m: ("func", m.group(1))),
    (r"^\s*" + _MOD + r"(?:var|let)\s+(\w+)\s*(?::|=)",
     lambda m: ("property", m.group(1))),
    (r"^\s*" + _MOD + r"init\??\s*\([^)]*\)\s*(?:\{|$)",
     lambda m: ("init", "init")),
    (r"^\s*" + _MOD + r"subscript\s*\([^)]*\)\s*(?:->.*?)?(?:\{|$)",
     lambda m: ("subscript", "subscript")),
    (r"^\s*" + _MOD + r"deinit\s*(?:\{|$)",
     lambda m: ("deinit", "deinit")),
    (r"^\s*(?:case\s+)(\w+)", lambda m: ("case", m.group(1))),
]


class _SwiftIndexBuilder:
    ...
    if mode == "index":
        ...
        return _(
            ...
            default=(
                ...
                "To retrieve a definition, call swift2idx with mode='section' and the section number."
            ),
            path=path,
            total=total,
            toc=toc,
