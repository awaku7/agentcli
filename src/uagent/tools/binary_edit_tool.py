            ...
            "properties": {
                ...
                "offset": {
                    ...
                    "description": _(
                        ...
                        default="Byte offset (0-based). Required for write/splice.",
                    ),
                },
                "data_hex": {
                    "type": "string",
                    "description": _(
                        ...
                        default="Hex string of bytes to write/insert (e.g. 'DEADBEEF' or 'DE AD BE EF').",
                    ),
                },
                "search_hex": {
                    "type": "string",
                    "description": _(
                        "param.search_hex.description",
                        default="Hex string to search for (replace mode).",
                    ),
                },
                "replace_hex": {
                    "type": "string",
                    "description": _(
                        "param.replace_hex.description",
                        default="Hex string to replace with (replace mode). Must be same length as search_hex unless allow_resize=true (not supported).",
                    ),
                },
                "occurrence": {
                    "type": "integer",
                ...
                "splice_op": {
                    ...
                    "enum": ["insert", "delete"],
                    "description": _(
                        "param.splice_op.description",
                        default="Splice operation (splice mode).",
                    ),
                },
                "delete_len": {
                    "type": "integer",
                    "description": _(
                        "param.delete_len.description",
                        default="Number of bytes to delete (splice delete).",
                    ),
                },
                "patch_json": {
                    "type": "string",
                    "description": _(
                        "param.patch_json.description",
                        default="JSON string patch (apply_patch mode).",
                    ),
                },
            },
            "required": ["path", "mode"],
...
def _ensure_file_ok(path: Path, *, max_bytes: int) -> tuple[int, str]:
    ...
    if not path.is_file():
        raise ValueError(f"[binary_edit] not a file: {path}")
...
def _op_write(data: bytearray, *, offset: int, patch: bytes) -> OpResult:
    if offset < 0:
        raise ValueError("[binary_edit] offset must be >= 0")
...
def _op_splice_delete(data: bytearray, *, offset: int, delete_len: int) -> OpResult:
    if offset < 0:
        raise ValueError("[binary_edit] offset must be >= 0")
    if delete_len <= 0:
        raise ValueError("[binary_edit] delete_len must be > 0")
...
def _parse_patch_json(patch_json: str) -> list[dict[str, Any]]:
    ...
    if not isinstance(obj, dict):
        raise ValueError("[binary_edit] patch_json must be a JSON object")
    ...
    try:
        ...
        if mode == "write":
            if "offset" not in args:
                raise ValueError("[binary_edit] offset is required for write")
            offset = int(args.get("offset"))
            patch = _hex_to_bytes(str(args.get("data_hex") or ""))
            if patch == b"":
                raise ValueError("[binary_edit] data_hex is required for write")
            ...
            if not search:
                raise ValueError("[binary_edit] search_hex is required")
            if not repl and repl != b"":
                raise ValueError("[binary_edit] replace_hex is required")
            ...
            if "offset" not in args:
                raise ValueError("[binary_edit] offset is required for splice")
            offset = int(args.get("offset"))
            splice_op = str(args.get("splice_op") or "").strip().lower()
            if splice_op not in ("insert", "delete"):
                raise ValueError("[binary_edit] splice_op must be 'insert' or 'delete'")
            if splice_op == "insert":
            ...
            if not patch_json.strip():
                raise ValueError("[binary_edit] patch_json is required")
