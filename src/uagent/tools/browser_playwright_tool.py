        ...
        try:
            for action in actions:
                ...
                if a_type == "switch_to_frame":
                    ...
                    _require_frame()
                    text = await current_frame.evaluate(
                ...
                # --- existing actions below ---
                    ...
                    results.append({"type": "handle_dialog", "action": dialog_action})
                    ...
                    await current_frame.goto(action["url"], wait_until=wu, timeout=to)
                    ...
                    factor = (
                        "window.innerHeight" if d == "down" else "-window.innerHeight"
                    )
                    ...
                    _require_frame()
                    elements = await current_frame.query_selector_all(
                        action["selector"]
                    )
                    attr_results = []
                    for el in elements:
                    ...
                    if c_path:
                        ...
                        results.append({"type": "content", "data": html})
            ...
            if intercepted_data:
                results.append(
                    {"type": "intercepted_network", "data": intercepted_data}
                )
            ...
            if trace_opts:
                ...
                await context.tracing.stop(path=t_path)
                results.append({"type": "trace", "path": str(Path(t_path).absolute())})
            ...
            return {
                "ok": False,
                "error": str(e),
                "last_url": page.url if page else None,
            }
