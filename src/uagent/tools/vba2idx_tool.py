...
_DEF_RE = re.compile(
    ...
    r"(Sub|Function|Property\s+(?:Get|Let|Set)|Type|Enum|Declare)\b\s*(.*)$",
    re.IGNORECASE,
)
_END_RE = re.compile(r"^\s*End\s+(Sub|Function|Property|Type|Enum)\b", re.IGNORECASE)
...
def run_tool(args: dict[str, Any]) -> str:
    ...
    try:
        ...
        if mode == "index":
            ...
            return "\n".join(rows) if rows else "No VBA definitions found."
        if mode == "section":
            try:
