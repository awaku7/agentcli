...
def _cmake_eval_condition(condition: str, variables: dict[str, str]) -> bool:
    condition = re.sub(
        r"\$\{([^}]+)\}", lambda m: variables.get(m.group(1), ""), condition
    ).strip()
    ...
    def atom(i):
        ...
        value = variables.get(
            token, token if token.lower() in ("on", "true", "yes", "y") else ""
        )
        return (
            str(value).lower() not in ("", "0", "false", "off", "no", "n", "notfound"),
            i + 1,
        )
...
def cmake_active_source(text: str) -> str:
    ...
    for raw in text.splitlines():
        ...
        if m:
            truth = _cmake_eval_condition(m.group(1), variables)
            active.append(active[-1] and truth)
            branch_taken.append(truth)
            continue
        m = re.match(r"elseif\s*\(\s*([^)]*)\)", line, re.I)
        if m and len(active) > 1:
            truth = _cmake_eval_condition(m.group(1), variables)
            active[-1] = active[-2] and (not branch_taken[-1]) and truth
            branch_taken[-1] = branch_taken[-1] or truth
            continue
        if re.match(r"else\s*\(", line, re.I):
            if len(active) > 1:
                active[-1] = active[-2] and not branch_taken[-1]
                branch_taken[-1] = True
            continue
        ...
        if active[-1]:
            expanded = re.sub(
                r"\$\{([^}]+)\}", lambda m: variables.get(m.group(1), m.group(0)), raw
            )
