    ...
    if manager in ("Maven", "Gradle") and ":" in name:
        ...
        for base in bases:
            for ver in versions or ["*"]:
                candidates.extend(
                    base.joinpath(*group.split("."), artifact, ver).glob("**/*")
                )
    elif manager == "NuGet":
        package_root = Path(
        ...
        candidates.extend(
            package_root.joinpath(
                name.lower(), version.lower() if version else "*"
            ).glob("**/*")
        )
        ...
        candidates.extend(
            [(root / ".build" / "checkouts") / Path(name).stem, home / ".swiftpm"]
        )
        ...
        for env_name in ("R_LIBS_USER", "R_LIBS_SITE"):
            for part in os.environ.get(env_name, "").split(os.pathsep):
                if part:
                    candidates.append(Path(part) / name)
    results = []
        ...
        try:
            ...
            for node in xml.iter():
                if (
                    ...
                    frameworks.extend(
                        x.strip().lower() for x in node.text.split(";") if x.strip()
                    )
...
def dependency_classpath_paths(
    dep: dict[str, Any], paths: list[str], root: Path
) -> list[str]:
    ...
    suffixes = {
        ...
        "SBT": {".jar"},
        "NuGet": {".dll", ".xml", ".winmd"},
        "SwiftPM": {".swiftmodule", ".swiftinterface"},
        "RubyGems": {".rb"},
        "Composer": {".php"},
        "DartPub": {".dart"},
        "LuaRocks": {".lua", ".so", ".dll"},
    ...
    for value in paths:
        ...
        if path.is_file():
            ...
            for suffix in suffixes:
                result.extend(
                    str(x.resolve()) for x in path.rglob(f"*{suffix}") if x.is_file()
                )
