...
BUSY_LABEL = True
STATUS_LABEL = "tool:echonet_property_set"
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "edt": {
                    "type": "string",
                    "description": _(
                        "param.edt.description",
                        default=(
                            "Optional raw EDT hex string to send. Takes precedence over value when provided."
                        ),
                    ),
                },
                "timeout": {
...
def run_tool(args: dict[str, Any]) -> str:
    ...
    if raw_edt is not None and str(raw_edt).strip():
        try:
            edt = bytes.fromhex(str(raw_edt).strip().replace(" ", ""))
        except Exception:
            payload = {
                "ok": False,
                "error": {
                    ...
                    "message": _(
                        "err.invalid_edt",
                        default="Error: edt must be a valid hex string.",
                    ),
                },
            }
            return (
        ...
        payload = {
            ...
            "error": {
                ...
                "message": _(
                    "err.value_required",
                    default="Error: either value or edt is required.",
                ),
            },
        }
        return (
