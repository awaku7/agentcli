    ...
    if not os.path.exists(path):
        if create_if_missing:
            return {"mcp_servers": []}, [
                _(
                    "warn.not_exists_new",
                    default="WARNING: {path!r} does not exist; treating as target for new creation.",
                ).format(path=path)
            ]
        return {"mcp_servers": []}, [
    ...
    if not servers:
        return "\n".join(
            ...
            + [
                _(
                    "err.empty",
                    default="ERROR: mcp_servers is empty, cannot change default.",
                )
            ]
        )
    ...
    if idx is None:
        return "\n".join(
            ...
            + [
                _(
                    "err.name_not_found",
                    default="ERROR: server_name={name!r} not found.",
    ...
    if idx == 0:
        return "\n".join(
            ...
            + [
                _(
                    "out.already_default",
                    default="OK: Already set as default: {name!r}",
                ).format(name=server_name)
            ]
        )
    ...
    try:
        try:
            ...
            with open(path, "w", encoding="utf-8") as f:
                f.write(text)
            return (
                _("out.ok", default="OK: Changed default: {name!r}").format(
                    name=server_name
                )
