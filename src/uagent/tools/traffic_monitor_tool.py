...
def _run_zeek(pcap_path: str, output_dir: str) -> Path:
    ...
    result = subprocess.run([zeek, "-r", pcap_path], cwd=out, capture_output=True, text=True, timeout=300, check=False)
...
def _parse_conn_log(path: Path, limit: int) -> list[dict[str, Any]]:
    ...
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        if not line or line.startswith("#") or fields is None:
            continue
        values = line.split("\t")
        events.append({field: values[index] if index < len(values) else None for index, field in enumerate(fields)})
        if len(events) >= limit:
            break
    return events
...
def run_tool(args: dict[str, Any]) -> str:
    ...
    try:
        if _find_zeek() is None:
            return _error("EXTERNAL_DEPENDENCY_MISSING", "zeek executable was not found.")
        log_path = _run_zeek(pcap_path, str(args.get("output_dir", "outputs/zeek")))
        events = _parse_conn_log(log_path, limit)
        return json.dumps({"ok": True, "backend": "zeek", "events": events, "returned_events": len(events)}, ensure_ascii=False)
...
TOOL_SPEC: dict[str, Any] = {
    ...
    "function": {
        ...
        "description": _("tool.description", default="Extract metadata-only traffic events from a pcap using Zeek."),
        "parameters": {
            "type": "object",
            "properties": {
