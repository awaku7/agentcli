...
BUSY_LABEL = True
STATUS_LABEL = "tool:matter_device_status"
            ...
            "properties": {
                ...
                "endpoint": {
                    ...
                    "description": _(
                        ...
                        default=(
                            "Optional endpoint filter. Use to narrow the returned endpoint data."
                        ),
                    ),
                },
                "output_format": {
...
def _collect_unique_clusters(item: dict[str, Any]) -> list[dict[str, Any]]:
...
def _collect_status(item: dict[str, Any]) -> dict[str, Any]:
    status = item.get("status") if isinstance(item.get("status"), dict) else {}
    if not status:
        status = item.get("raw") if isinstance(item.get("raw"), dict) else {}
    output: dict[str, Any] = {}
    for key in (
        "power",
        "onOff",
...
def _normalize_endpoint_filter(endpoint: str | None) -> str | None:
    if endpoint is None:
        return None
...
def _format_text(result: dict[str, Any]) -> str:
    ...
    dev_attrs = device.get("device_attributes") or {}
    if dev_attrs:
        lines.append(
            "Device attributes: {}".format(
                ", ".join(f"{k}={v}" for k, v in sorted(dev_attrs.items()))
            )
        )
        ...
        for endpoint in endpoints:
            ...
            lines.append(
                "- endpoint {eid}{loc} type={dtype} clusters={count}".format(
                    ...
                    count=len(endpoint.get("clusters") or []),
                )
            )
    ...
    if status:
        lines.append(f"Status keys: {', '.join(sorted(status.keys()))}")
    return "\n".join(lines)
...
def run_tool(args: dict[str, Any]) -> str:
    ...
    if endpoint:
        ...
        endpoints = [
            ep
            for ep in endpoints
            if str(ep.get("endpoint_id") or "").casefold() == endpoint_key
            or endpoint_key in str(ep.get("endpoint_id") or "").casefold()
        ]
    ...
    result = {
        ...
        "status": _collect_status(item),
        "endpoints": endpoints,
        "clusters": _collect_unique_clusters({**item, "endpoints": endpoints}),
        "fetched_at": _now_iso(),
        "raw": {
            "source": item.get("source"),
        },
