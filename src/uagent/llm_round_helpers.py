...
while True:
    try:
        ...
        msg = str(e)
        # Detect token limit / context length exceeded errors
        if (
            "exceeds the maximum number of tokens" in msg
            or "400 INVALID_ARGUMENT" in msg
            ...
            print(
                _(
                    "[WARN] Gemini token limit exceeded. Attempting to shrink conversation history..."
                ),
                file=_sys.stderr,
            )
            if messages is not None and core is not None:
                ...
                new_msgs = shrink_messages(messages, keep_last=keep_last)
                messages.clear()
                ...
                from .llm_round_helpers import _translate_call_messages
        ...
        if action == "give_up":
            ...
            print(repr(e))
            return False, client, "", [], {}, gemini_cache_name
...
while True:
    try:
        ...
        if (
            ...
            print(
                _(
                    "[WARN] Claude token limit exceeded. Attempting to shrink conversation history..."
                ),
                file=_sys.stderr,
            )
            if messages is not None and core is not None:
...
while True:
    try:
        ...
        # Detect token limit / context length exceeded errors
        if (
            "context window" in msg.lower()
            or "exceeds the context" in msg.lower()
            ...
            print(
                _t(
                    "[WARN] OpenAI/Azure token limit exceeded. Attempting to shrink conversation history..."
                ),
                file=_sys.stderr,
            )
            if messages is not None and core is not None:
