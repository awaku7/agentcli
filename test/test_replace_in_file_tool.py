...
class TestReplaceInFileTool(unittest.TestCase):
    ...
    def test_backup_rotation(self):
        ...
        self.assertTrue(os.path.exists(self.test_abs_path + ".org"), "Backup .org missing")
        self.assertTrue(os.path.exists(self.test_abs_path + ".org1"), "Backup .org1 missing")
        self.assertTrue(os.path.exists(self.test_abs_path + ".org2"), "Backup .org2 missing")
    ...
    def test_dangerous_path_rejection(self):
        ...
        result = json.loads(run_tool(args))
        self.assertFalse(result["ok"])
        self.assertTrue("dangerous path" in result["error"] or "not allowed" in result["error"])
    ...
    def test_python_string_literal_newline_injection_breaks_syntax(self):
        """Pythonコード中の通常文字列リテラルに実改行が混入すると構文が壊れることを再現する。"""
    ...
    def test_invalid_regex_pattern(self):
        ...
        self.assertTrue(any(x in err for x in ["unterminated", "invalid", "patternerror"]))

    def test_regex_replacement_with_newline_injection_can_break_python_string_literal(self):
        """regex置換で replacement に実改行を入れると、.py内の通常文字列が壊れ得ることを再現"""
