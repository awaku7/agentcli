...
- 暗号化された`.env.sec`から環境変数を削除する`uag_envsec delete`を追加。

### 変更

- 暗号化環境変数の管理方法を英語・日本語のガイドに追記。
...
- `llmcapa`によるMeta Responsesのケイパビリティ対応を追加。
- 画像生成および画像間変換ルートで選択されたAPIモードを報告。

### 変更

- 画像ツール間で画像出力形式の処理を共通化。
- 対応する`llmcapa`の最低バージョンを`>=0.5.29`に更新。

### 修正

- Responses APIのケイパビリティに基づく画像ルートとメインラインモデルの選択を修正。
...
- Meta Responsesの推論対応とResponses API処理の改善を追加。
- 管理ツールのロードループに対する保護を追加。
- 対話出力を改善するスピナー／ランタイム対応を追加。

### 変更

- セッション一覧・読み込み・検索ビューを統合し、新しいメッセージをローカライズ。
- Gemini/Vertexカタログ処理を拡張し、翻訳を更新。
...
- セッション所有Artifactの保存、範囲指定読み取り、メタデータ確認、バイナリ安全なエクスポートを追加。
- コンテキスト境界のドキュメントを追加し、ツール結果の永続化とSQLiteセッション処理を強化。

### 変更

- 翻訳時もCLIコマンド構文を保持し、説明文だけを翻訳するよう変更。
- 管理ツール以外をデフォルトで遅延ロードし、初期ロード対象を `tool_catalog`、`tool_load`、`unload_tool` に限定。
- `llmcapa` の最低バージョン要件を `>=0.5.17` に更新。
...
- Responses APIを利用したMeta画像ワークフローとUnicode安全な画像処理を追加。
- 関連するツール呼び出し間でペイロード以外の状態を共有するツールコンテキストストアを追加。

### 変更

- 実行時および開発用の依存関係を `pyproject.toml` のextrasに集約し、`requirements.txt` を簡素化。
- `llmcapa` の最低バージョン要件を `>=0.5.15` に更新。
- Linuxではファイルベースの秘密情報ストアと資格情報ストアをデフォルトで使用。
- CLIへの貼り付けテキストでCRLF/CR改行を正規化。

### 修正

- OS keyringとSecret Service呼び出しが起動を無期限にブロックしないよう修正。
- Responses APIの圧縮通知をストリーミングコンソール出力と同期。

## [0.6.18] - 2026-09-01

### 追加

- Brainメモリ設計ドキュメントを追加し、冗長なアイドル状態通知を抑制。
- ExcelからMarkdownへの変換とプロジェクトバッジを改善。
- PyPIトークンの取り扱いを文書化し、XLSMの国際化対応を改善。
...
- ワークシート、数式、依存関係、VBAを解析するOffice／XLSM解析ツールを追加。

### 変更

- LM StudioのURL処理とコンテキスト上限処理を改善。
- プロバイダー認証ラベルとプロンプト状態表示を改善。
- Office解析ツールをBlackでフォーマット。
- `GITHUB_REPO`を廃止し、ローカルの`origin`リモートからGitHubリポジトリを取得。

### 修正

- APIキーなしでLM Studioを起動できるよう修正。
- XLSM VBA解析で`oletools`を自動インストールできるよう修正。

## [0.6.16] - 2026-08-30

### 追加

- ツールガードと永続タスク履歴に対応したスケジューラー直接実行機能を追加。
- DeepL翻訳プロバイダー対応と関連ドキュメント・テストを追加。
...
- Chat Completions、Responses、SDKトランスポートに対応するLM Studioプロバイダーを追加。
- LM Studio SDK経由のツール実行とストリーミングに対応。
- ツール名・引数を対象とするSQLiteセッション検索のテストを追加。

### 変更

- 読み取り専用ツールを含むツール名とマスク済み引数の表示を復旧。
- LM Studio SDKのツール呼び出し履歴を保持し、SDK結果から表示可能なテキストを抽出。
- セッション検索結果に日時、プロジェクト、エントリーポイントを表示。

### 修正

- セッション読み込み時に中断されたSQLiteツール呼び出し履歴を安全に復元。
- LM Studio SDKプロバイダー依存関係の自動インストールを有効化。

## [0.6.14] - 2026-08-28

### 追加

- SVGをPNG、JPEG、WebPへ変換する画像変換ツールを追加。
- Mermaid MarkdownをSVGへ変換するツールを追加。
- CairoSVGをtoolsオプション依存関係として追加。

### 変更

- 副作用を持つツールの引数表示を、確認・実行前の共通処理へ集約。
...
- 再開可能なタスク情報とイベント履歴に対応するSQLiteベースのバッチタスク履歴を追加。
- コードマップのオントロジー出力に、意味的な呼び出し・継承関係、語彙宣言、安定した識別子を追加。
- R、Objective-C、COBOL向けのTree-sitterシンボル抽出を追加。

### 変更

- 外部ファイル対応とimport関係の識別子安定化を含め、コードマップのパス処理を改善。
- SQLiteタスク履歴ワークフロー向けに、バッチ状態のローカライズと開発者ドキュメントを拡充。
- 履歴圧縮で使用するメッセージの生成ロケール対応を改善。
...
- envsecマスターキーのOSキーリング移行と、ツール単位のコンテキスト保存仕様を追加。

### 変更

- envsecキーリングを優先し、移行成功後に旧キー ファイルを削除。キーが異なる場合も有効なファイルキーをキーリングへ保存して移行。
- 従来のツール先読み動作を維持し、`UAGENT_TOOLS_WARMUP=0` でオプトアウト可能に変更。
- ツール実行後のGCをオプトイン化し、`UAGENT_GC_AFTER_TOOL=1` で有効化。
- 中央管理されたツール実行中は、ツール内部による一時的なIDLE通知を抑制。
- `llmcapa>=0.5.12` を要求。

### 修正

- ツール実行後、LLM応答中に入力プロンプトが一時的に入力を受け付ける問題を修正。

## [0.6.11] - 2026-08-26

### 追加

- Auto-Pilotおよび注入モードで、ツール単位・MCP機能単位の `absconfirm` 承認ゲートを追加。
- 自律的な実機操作における安全な確認パターンを文書化。

### 変更

- セッション要約を先頭10件に制限し、終了時要約をオプトイン化。
- コマンド・テスト実行のタイムアウト、Windowsプロセス分離、非対話テスト実行時の安全策を追加。
- Auto-Pilotのループ検出、注入モード通知、ローカライズメッセージ、生成済みロケールカタログを改善。

### 修正

- MCPのHTTP 406プローブエラーを、誤ったレガシー初期化フォールバックなしで明示的に通知。

## [0.6.10] - 2026-08-25

### 追加

- Markdownの見出しを参照アウトラインに合わせて並べ替える `mdreorder_tool` を追加。

### 変更

- 読み込み直したセッションを「最近使用したセッション」として更新し、以降のメッセージやツール結果がそのセッションに紐づくように変更。
