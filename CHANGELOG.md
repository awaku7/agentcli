...
- Dynamic tool catalog: `tool_catalog` / `tool_load` for runtime discovery without loading all tools upfront.
  - UI genre checkbox toggle in GUI/Web/A2A.
  - README i18n: translated multi-provider and tool catalog features into 30 languages.
- i18n: complete 30-language coverage for all tool JSON specs.
  - Added bn, fa, mn, mr translations to shared JSON files.
  - Normalized all tool JSON files to 30 languages with missing keys from Python code.
  - Translated vision_runtime.json (10 backend/prompt keys) into 29 languages.
  - Translated 6 tool JSON files (bluesky, switchbot_batch, usb_camera, vision_deepseek, vision_ollama, vision_openai).

### Fixed
- ANSI escape corruption under cmd.exe cp932: call `SetConsoleOutputCP(65001)` in `_reconfigure_stdio()`.
