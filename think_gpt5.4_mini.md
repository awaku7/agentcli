...
2. LLM 呼び出し前に `current_file` を 1 件だけ注入する
3. LLM に見せるツールを allowlist で絞る
4. 応答後に `current_file` 以外を触っていないか検証する
5. 成功したら `complete_file`、失敗なら `skip_file` / `error_file` を呼ぶ
