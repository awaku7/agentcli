<script>
  ...
  import { getModes, getGenresEnabled, getStatus, sendSetModes, toggleGenre, sendCommand, fetchToolsEnabled, setToolsEnabled, fetchMemories, addMemory, updateMemory, deleteMemory, fetchLogs, fetchProfile, clearProfile, profileFromLogs, updateProfile } from '../../lib/stores.svelte.js';
      ...
      <div class="p-5 space-y-4 overflow-y-auto">
        ...
        <div class="flex items-center justify-between pt-2 border-t" style="border-color:var(--border-color);">
          ...
          <button onclick={doToolsToggle} class="relative w-12 h-6 rounded-full transition cursor-pointer"
            style="background:{toolsEnabled ? 'var(--accent)' : 'var(--border-color)'};"
          ><span class="absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition" style="left:{toolsEnabled ? 'calc(100% - 22px)' : '2px'};"></span></button>
        </div>
      </div>
