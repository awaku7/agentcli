...
- **183 инструмент**: файловый ввод-вывод, веб-поиск, генерация изображений, Gmail, сканирование устройств BLE, интеграция с сервером MCP — **111 из них безопасны для параллельного выполнения** (до 8 выполняются одновременно через пул потоков, настраивается через `UAGENT_PARALLEL_WORKERS`). Когда LLM запускает несколько вызовов инструментов одновременно, uag автоматически распараллеливает их.
