...
- **195 инструмент**: файловый ввод-вывод, веб-поиск, генерация изображений, Gmail, сканирование устройств BLE, интеграция с сервером MCP — **111 из них безопасны для параллельного выполнения** (до 8 выполняются одновременно через пул потоков, настраивается через `UAGENT_PARALLEL_WORKERS`). Когда LLM запускает несколько вызовов инструментов одновременно, uag автоматически распараллеливает их.
...
| **Инструменты для разработчиков** | git_ops, python_compile, lint_format, run_tests, db_query, **26 навигаторов исходного кода (семейство idx)** |
...
| **Навигация по источнику** | **26 инструментов idx** для Python, PHP, TypeScript, Java, C#, Dart, C/C++, Rust, Go, Swift, Kotlin, COBOL — получите индекс функции/класса или конкретное определение, не читая весь файл |
