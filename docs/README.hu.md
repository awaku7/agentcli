...
- **229 eszköz**: Fájl I/O, webes keresés, képgenerálás, Gmail, BLE-eszközök szkennelése, MCP-szerver integráció – **130 párhuzamosan biztonságos** (akár 8 végrehajtása párhuzamosan a szálkészleten keresztül, az `UAGENT_PARALLEL_WORKERS`-en keresztül konfigurálható). Amikor az LLM egyszerre több eszközhívást indít el, az uag automatikusan párhuzamosítja azokat.
...
130 eszköz `x_parallel_safe` megjelöléssel rendelkezik, és egyidejűleg fut a `ThreadPoolExecutor'-on keresztül (alapértelmezés szerint 8 szál; állítsa be az `UAGENT_PARALLEL_WORKERS\` paramétert a módosításhoz).
...
Az A2A életciklusát, az I18N-kontextusokat, az opcionális függőségek telepítését, az eszközbiztonságot, a szolgáltatói képességeket, az OAuth bizalmi határait, a strukturált eseményeket és az elfogadási ellenőrzést lefedő tartós megvalósítási szerződésekért lásd az [ARCHITECTURE.md](ARCHITECTURE.md) dokumentumot.
