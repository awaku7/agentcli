...
親プラン: [`../N8N_ADAPTATION_PLAN.md`](../N8N_ADAPTATION_PLAN.md)
...
2. **Connection details** を開き:
   - Instance MCP の **URL** を控える
   - **Access Token** を発行し、表示中にコピー（再表示されない）
3. 検証用ワークフローを 1 本作り、**Available in MCP** を ON
4. 可能なら description を書く（エージェントの検索用）
...
4. **Execute workflow** を実行
5. `Extract task fields` で `task_status=SUCCEEDED` と `assistant_content` を確認
