...
<p align="center">
  ...
  ·
  <a href="README.translations.md">用您的语言阅读本文</a>
</p>
...
- **112 种工具**：文件 I/O、网络搜索、图像生成、BLE 设备扫描、MCP 服务器集成 — 并且 **其中 66 种工具并行运行**。当 LLM 一次触发多个工具调用时，uag 会通过线程池自动执行它们。
- **4 UI + A2A**：CLI、GUI、Web 和代理到代理协议。相同的引擎，任何接口。
...
有关所有环境变量，请参阅 [ENVIRONMENT.md](../ENVIRONMENT.md))。

## 特点

### 🧠 多提供商架构

OpenAI / Azure / Bedrock / OpenRouter / Ollama / Gemini / Vertex AI / Claude / Grok / NVIDIA / DeepSeek / HuggingFace / 阿里云 (Qwen) / KIMI (Moonshot AI) / 小米 MiMo / LM Studio / MiniMax / **Sakana AI (Fugu)**
...
设置`UAGENT_LANG`进行切换。请参阅 [ADD_LOCALE.md](../src/uagent/docs/ADD_LOCALE.md)) 添加新的区域设置。
...
- **响应 API**：为响应 API 模式设置“UAGENT_RESPONSES=1”（OpenAI/Azure/Bedrock/OpenRouter/Ollama/Alibaba/LM Studio/Sakana AI）
