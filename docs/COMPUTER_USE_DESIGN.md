...
agentcli側では、現在の依存下限であるllmcapa 0.5.27以降が提供する `ComputerUseCapability` を正規情報源として利用する。llmcapaはComputer Use機能に必須の依存関係とし、Computer Useを有効にする構成では必ずインストールする。UAG側にモデル情報のコピーや独自レジストリは持たせない。llmcapaが利用できない環境では、Computer Useを対応不可として明示的に停止する。
...
`llmcapa 0.5.27` をComputer Use capabilityの正規情報源とする。UAG側で同じモデル情報を独自管理しない。
