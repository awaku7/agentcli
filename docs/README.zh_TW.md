...
- **隨時退出**：按 **F11** 鍵停止自動駕駛。**F12** 只會停止目前的 LLM 回應。或讓審閱者決定何時達到目標。
...
按 **F11** 鍵退出自動駕駛模式。**F12** 只會停止目前的 LLM 回應（請參閱 [README_AUTO.md](https://github.com/awaku7/agentcli/blob/main/docs/README_AUTO.md)）。
