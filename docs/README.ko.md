...
- **언제든지 종료**: 응답 도중이라도 즉시 중지하려면 F11 키를 누르세요. 또는 검토자가 목표 달성 시기를 결정하도록 합니다.
...
| 인터페이스 | 방해하는 방법 |
|---|---|
| **CLI** | LLM 스트리밍 중에 F12 키를 누르세요. 현재 응답이 중지되고 `"Stop"`이 사용자 메시지로 전송되어 LLM이 그에 따라 응답합니다 |
...
자동 조종 모드를 종료하려면 F11 키를 누르세요. [README_AUTO.md](https://github.com/awaku7/agentcli/blob/main/docs/README_AUTO.md)).
