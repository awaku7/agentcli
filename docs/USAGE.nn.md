...
Dette dokumentet beskriver kommandolinjealternativene som er tilgjengelege for uag-inngangspunkter.
...
Arbeidskatalog. Hvis ikkje angitt, faller systemet tilbake til miljøvariabelen `UAGENT_WORKDIR`, deretter den gjeldande katalogen.
Katalogen opprettes hvis den ikkje finnes.
...
Når den er deaktivert, mottar LLM ingen verktøydfinisjoner og kan ikkje kalle opp noe verktøy.
...
Start autopilot fra et ikkje-interaktivt injisert mål. Verdien brukar de samme alternativene som `:auto`; sett hele verdien i anførselstegn når den inneholder alternativer.
...
Normalmodus brukar vurderingsveien til anmelderen. Sett `UAGENT_AUTO_SENTINEL=1` for å velge enkel-LLM vaktmodus. I denne modusen må målet LLM avslutte hvert svar med nøyaktig én av følgende:

- `<AUTO_CONTINUE>` — kjør en ny runde
- `<AUTO_COMPLETE>` — fullfør med suksess

Manglende eller ugyldige markører stopper autopiloten på en sikkjer måte. Dette kjører fortsatt målet `LLM`; det unngår berre det ekstra `LLM`-kallet til granskeren.

### `--non-interactive`

Ikke-interaktiv modus. Starter ikkje stdin-sløyfen. Hvis en filbane angis som et posisjonsargument, behandles den, og programmet avsluttes umiddelbart.
...
Som standard lytter webserveren kun på localhost (`127.0.0.1`). For å gjøre den tilgjengeleg fra andre maskiner på nettverket, bruk `--host 0.0.0.0`.
...
| `UAGENT_CONSECUTIVE_TOOL_CALL_LIMIT` | Maksimalt antall påfølgende nye verktøykall (standard: `50`) |
