...
2. **n8n → uag**: HTTP で A2A（`/message:send`）を呼ぶ
3. 足りない操作だけ REST ラッパ（`n8n_*` ツール）を追加する
...
2. 対象 WF で **Available in MCP** を ON（または MCP 設定画面から Enable）
3. Connection details から **Access Token** を発行（初回のみ平文表示）
4. WF description を充実（エージェントの発見性向上）
...
2. 検証 WF のみ Available in MCP
3. Access Token を発行して安全な場所へ
4. （逆方向）uag の URL と Bearer を HTTP Request に設定

### uag

1. `mcp_servers` に n8n / n8n-docs を追加
2. `mcp_tools_list` → ツール名を確認
3. `handle_mcp_v2` で search / execute
4. A2A を起動し token を設定
5. 秘密をコミットしていないか確認
...
2. ~~**A2A ping WF**~~ → 済: `docs/n8n/a2a_ping.workflow.json` + `docs/n8n/README.md`
3. **Phase 0 を実インスタンスで実施** — ログを残す（手順は `docs/n8n/README.md`）
4. **ギャップが出た場合のみ** `n8n_tool.py` の API 設計 → 実装
