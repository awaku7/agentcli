...
- **112 のツール**: ファイル I/O、Web 検索、画像生成、BLE デバイス スキャン、MCP サーバー統合 — **66 が並列実行対応**（最大4つまで同時実行）。 LLM が複数のツール呼び出しを同時に起動すると、uag はスレッド プールを介して自動的に並列化します。
...
66 のツールには「x_Parallel_safe」のマークが付けられ、4 スレッドの「ThreadPoolExecutor」を通じて同時に実行されます。
