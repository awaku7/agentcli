...
<p align="center">
  ...
  ·
  <a href="README.translations.md">এটি আপনার ভাষায় পড়ুন</a>
</p>
...
- **131 টুল**: ফাইল I/O, ওয়েব সার্চ, ইমেজ জেনারেশন, BLE ডিভাইস স্ক্যানিং, MCP সার্ভার ইন্টিগ্রেশন — এবং **76টি সমান্তরালে চলে**। যখন LLM একাধিক টুল কল একবারে ফায়ার করে, uag স্বয়ংক্রিয়ভাবে একটি থ্রেড পুলের মাধ্যমে সেগুলি চালায়।
- **4 UI + A2A**: CLI, GUI, ওয়েব এবং এজেন্ট-টু-এজেন্ট প্রোটোকল। একই ইঞ্জিন, যেকোনো ইন্টারফেস।
...
সমস্ত পরিবেশের ভেরিয়েবলের জন্য [ENVIRONMENT.md](../ENVIRONMENT.md)) দেখুন।

## বৈশিষ্ট্য

### 🧠 মাল্টি-প্রোভাইডার আর্কিটেকচার

OpenAI / Azure / Bedrock / OpenRouter / Ollama / Gemini / Vertex AI / Claude / Grok / NVIDIA / DeepSeek / HuggingFace / Alibaba ক্লাউড (Qwen) / KIMI (Moonshot AI) / Xiaomi MiMo / LM স্টুডিও
...
76টি টুলকে `x_parallel_safe` হিসেবে চিহ্নিত করা হয়েছে এবং একটি 4-থ্রেড `ThreadPoolExecutor` এর মাধ্যমে একযোগে চালানো হয়।
...
| **দেব টুলস** | git_ops, python_compile, lint_format, run_tests, db_query, **13 idx tools** |
...
স্যুইচ করতে `UAGENT_LANG` সেট করুন। একটি নতুন লোকেল যোগ করতে [ADD_LOCALE.md](../src/uagent/docs/ADD_LOCALE.md)) দেখুন।
...
- **পরিবেশ ভেরিয়েবল**: [ENVIRONMENT.md](../ENVIRONMENT.md))
- **সেটআপ উইজার্ড**: `python -m uagent.setup_cli`
- **এনক্রিপ্ট করা env**: `uag_envsec` — `.env` কে `.env.sec` হিসেবে এনক্রিপ্ট করুন
- **প্রতিক্রিয়া API**: প্রতিক্রিয়া API মোডের জন্য `UAGENT_RESPONSES=1` সেট করুন (OpenAI/Azure/Bedrock/OpenRouter/Ollama/Alibaba/LM Studio / MiniMax / **Sakana AI (Fugu)**)
