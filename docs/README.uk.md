...
- **171 інструмент**: введення/виведення файлів, веб-пошук, створення зображень, Gmail, сканування пристроїв BLE, інтеграція сервера MCP — **89 є паралельно безпечними** (до 8 виконуються одночасно через пул потоків, налаштовується за допомогою `UAGENT_PARALLEL_WORKERS`). Коли LLM запускає кілька викликів інструментів одночасно, uag автоматично розпаралелює їх.
...
89 інструментів позначено як `x_parallel_safe` і виконуються одночасно через `ThreadPoolExecutor` (8 потоків за замовчуванням; змініть `UAGENT_PARALLEL_WORKERS`).
