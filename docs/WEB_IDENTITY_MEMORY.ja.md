...
固定Project運用では `UAGENT_MEMORY_PROJECT` がserver-side bindingになります。固定していないOIDC session、またはProjectContext cookieを使う認証済みnon-OIDC sessionでは、認証後に次でProjectを選択します。
