...
**いいえ。** 現行の標準 registry 経路には `pfn` と `grok` が追加済みで、通常の route はそれぞれ `PfnProviderRuntime` と `GrokGrpcProviderRuntime` を使う。一方、互換用の legacy round registry には現時点で次の provider が残る。

- `gemini`, `vertexai`, `claude`
- `deepseek`, `mimo`
- `zai`, `novita`
- `vercel`, `together`

さらに、registry が対象外の既知 provider には OpenAI-compatible の legacy fallback がある。したがって「Grok/PFN だけが legacy に残っている」という状態ではなく、**Grok/PFN は registry へ移行済みだが、旧 wrapper と fallback を互換境界として残している**状態である。

### 115.2 「なぜ legacy を残すのか」

legacy を残している理由は、新設計を標準経路に戻すためではなく、次の互換条件を一度に壊さないためである。

1. provider 固有 SDK / transport（Grok の xAI gRPC、PFN の stream/tool schema、その他 legacy provider の専用 API）を切り替えた際の rollback point を維持する。
1. 既存の tuple result、host callback、tool continuation、Responses state の保存形式を利用する呼び出し元を段階的に移行する。
1. 実 API の stream event、rate-limit retry、tool error、再送・中断の characterization を provider ごとに確認してから fallback を削除する。
1. registry adapter が利用できない環境、明示的な transport override、未対応 model で従来経路を失わないようにする。

このため legacy は「新機能を追加する場所」ではなく、**削除条件を満たすまでの compatibility adapter** として扱う。新しい判断（capability、transform、retry、renderer、tool continuation）を legacy に再実装してはならない。

### 115.3 「legacy に残したくない場合の削除条件」

legacy wrapper / fallback を削除する場合は、provider ごとに次を満たした後に限る。

- registry route が通常経路であることを route test で固定する。
- Chat/Responses、stream/non-stream、tool call、tool result、retry、interrupt/cancel、empty round、provider switch の characterization test があること。
- 旧 tuple / host callback / session state を registry の `RoundResult` / `StreamEvent` へ変換する互換テストがあること。
- 実 API または同等の fixture で認証・rate limit・再送・異常 event を確認し、旧経路との差分を記録すること。
- 明示的な transport override と rollback 手順を廃止できることをリリースノートに記録すること。

この条件を満たさずに `llm_grok_round.py`、PFN の legacy wrapper、または legacy round registry を削除すると、利用可能性ではなく「未検証の切断」になる。したがって現時点では、Grok/PFN の legacy wrapper は削除対象ではなく、registry route の検証完了後に削除する候補である。

### 115.4 実測報告の各指摘への現在回答

| 報告の指摘 | 現行回答 | 状態 |
| --- | --- | --- |
| `--inject-message-auto` の引用符・改行が壊れる | `util_cmd_auto.py` を suffix option parser に変更し、goal 本文をそのまま保持する回帰テストを追加 | 対応済み |
| 同一 tool の無進展ループ | 連続 tool call / management tool loop の検出を現行実装で確認 | 確認済み |
| `.env.sec` と明示 `UAGENT_*` の優先 | explicit environment を保持する初期化経路を現行実装で確認 | 確認済み |
| MCP の `server_name` エラー案内 | 未設定・引数欠落・利用可能 server 一覧の提示を改善する | 次の修正候補 |
| embedded tool allowlist / `bash_exec` 制御 | embedded の tool genre mask の扱いと危険 tool の明示的 disable / allowlist を整理する | 未完了 |
| sentinel と tool-call-only 応答 | tool call がある応答を text-only sentinel 判定と混同しない契約を固定する | 未完了 |
| MCP inputSchema の未知引数 | strict 検証と tolerant mode の境界を定義する | 未完了 |
| tool result の永続ログ | `[TOOL-RESULT]` 相当の要約を secret masking 付きで durable log に残す | 未完了 |
| reviewer への tool result | reviewer/judgment 用 context に compact な tool result summary を含める | 未完了 |
| round summary / telemetry | host が利用できる構造化 round summary を標準出力・session log と分離して定義する | 未完了 |
| 外部 kill 時の MCP cancellation | host 終了・cancel と MCP request cancellation の境界を実装する | 未完了 |

この表は、今後の修正を行うたびに対応済み / 確認済み / 未完了を更新する。報告の再現用に復号した一時ファイルは保持せず、秘密情報と実機接続情報は本書へ転記しない。
