...
外部の自動操縦実測報告（0.6.11 / 2026-09-17）との差分として、`util_cmd_auto.py` の `--inject-message-auto` / `:auto` 引数解析を shell tokenization から suffix option parser へ変更し、改行・引用符・アポストロフィを保持する回帰テストを追加した。現行コードで継続確認が必要な項目は、MCP `server_name` エラーの案内改善、embedded tool allowlist と `bash_exec` 制御、sentinel と tool call の相互作用、MCP inputSchema による未知引数検証、tool result の永続ログ、reviewer への tool result 要約である。`RoundSummary` は registry 経路の `RoundResult` に追加され、イベント数・文字数・推定入力トークン・投影サイズ・tool schema サイズ・retry/recovery 情報・usage delta を秘密情報なしで取得できる。残る観測性の課題は、legacy 全経路への summary 接続と、GUI/Web への `[TOOL-RESULT]` 表示投影である。`ContextResultManager` には raw result を含まない bounded な `ui_display` projection を追加済みで、CLI は `UAGENT_SHOW_TOOL_RESULTS=1` を明示した場合の表示まで配線済みである。Web/GUI では表示しない。MCP cancellation 後に到着する late response について、完了直後にも cancel 状態と request generation を再検証し、`MCP_STALE` として tool continuation に渡さない初期対応を追加した。generation は host callback と SessionStore の AgentState 経由で同一 session に保持し、session restore path でも AgentState を core に再適用する。provider / model が復元値と一致しない場合は generation を invalidate して安全側へ進める。fresh core を process boundary とみなす restore integration test（`tests/test_mcp_generation_restore_integration.py`）を追加し、復元した Responses continuation と MCP generation が host cancel 後も stale guard を維持することを確認した。実プロセス再起動を使う restore 検証まで完了した。残る課題は、複数 round / host kill をまたぐ実 transport の stale continuation 統合テストと、host renderer への配信接続である。一方、同一 tool の無進展ループ検出と `.env.sec` の明示的な `UAGENT_*` 環境変数優先は現行実装に存在するため、報告時点との差分確認済みとして扱う。なお、stdin が開いた状態で CLI 起動から初回 LLM 実行まで45〜90秒遅延する報告は、tool 子プロセスの stdin 継承とは分離し、`UAGENT_STARTUP_TIMING=1` と stdin リダイレクト比較で再現確認する未確定課題として記録する。
