...
部分完了   R1    nvidia / openrouter / moonshot / alibaba / sakana / bedrock / ollama / lmstudio / meta / pfn の registry 化、残り provider の標準経路化は未完了
...
現行の registry 所有範囲は `providers/runtime_registry.py` の `_SUPPORTED` に集約され、次の13 providerである。

| 経路 | provider | 現状 |
| --- | --- | --- |
| registry runtime | `openai`, `azure`, `inception`, `nvidia`, `openrouter`, `moonshot`, `alibaba`, `sakana`, `bedrock`, `ollama`, `lmstudio`, `meta`, `pfn` | `OpenAICompatibleRuntime`、`InceptionProviderRuntime`、または `PfnProviderRuntime` を構築可能。`uagent_llm.py` の route 判定から利用する。`nvidia` / `moonshot` は Chat Completions、`openrouter` / `alibaba` / `sakana` / `bedrock` / `ollama` / `lmstudio` / `meta` は provider-specific な request 正規化、`pfn` は専用 tool/stream event 正規化を適用する（LM Studio SDK transport は除外） |
...
`uagent_llm.py` の実行順序は、registry → legacy round registry → OpenAI-compatible fallbackで固定されている。registry の eligibility 判定は `resolve_registry_round_route()` に集約済みで、registry 所有範囲は13 providerに拡張された。`tests/test_provider_runtime_registry.py` と `tests/test_openai_compatible_runtime.py` は `nvidia` / `openrouter` / `moonshot` / `alibaba` / `sakana` / `bedrock` / `ollama` / `lmstudio` / `meta` / `pfn` の adapter と未移行 providerの拒否を固定している。
...
`nvidia` と `moonshot` は専用 legacy round handler と Responses 固有変換を持たない OpenAI-compatible providerとして、Chat Completions の request、tool call、stream event の generic adapter characterization を通過したため `_SUPPORTED` に追加した。`alibaba` と `sakana` は OpenAI-compatible Responses の request / completion 正規化 characterization、`bedrock` は専用の transcript flattening と flat tool request、`ollama` は Chat/Responses の extra-body compatibility、`lmstudio` は REST Chat/Responses transport、`meta` は Responses reasoning summary の compatibility characterization を通過したため同じく追加した。LM Studio SDK transport は registry 対象外とした。PFN/PLaMo は専用の tool schema、catalog steering、embedded tool-call fallback、非ストリーミング tool round 契約を `PfnProviderRuntime` に隔離し、pure stream parser と StreamEvent 変換の characterization tests を通過したため `_SUPPORTED` に追加した。Grok の legacy xAI gRPC / SDK 経路と host callback の characterization tests は成功したが、registry runtime は OpenAI-compatible request と異なる SDK・stream/tool 契約を持つため `_SUPPORTED` には追加せず legacy に留める。REST override（`UAGENT_GROK_USE_XAI_SDK=0`）も専用 message/tool 変換の比較が必要である。

Grok を legacy に残すのは未対応のためではなく、汎用 registry の標準 OpenAI request / stream / tool-call 契約に収まらない xAI gRPC / SDK 契約を互換境界へ隔離するためである。PFN は専用 `PfnProviderRuntime`、pure stream parser、StreamEvent 変換、retry boundary の characterization を完了し、registry 経路へ移行した。ただし既存 `llm_pfn.py` の legacy wrapper と tool continuation は互換性のため当面残す。
