...
<p align="center">
  Operações de ficheiros / Pesquisa na web / Geração e análise de imagens / Extração de PDF e Excel / Controle de IoT / Integração de MCP<br>
...
- **229 ferramentas**: E/S de arquivos, pesquisa na web, geração de imagens, Gmail, verificação de dispositivos BLE, integração de servidor MCP — **130 são seguras em paralelo** (até 8 são executadas simultaneamente por meio de pool de threads, configuráveis ​​via `UAGENT_PARALLEL_WORKERS`). Quando o LLM dispara várias chamadas de ferramenta ao mesmo tempo, o uag as paraleliza automaticamente.
...
[pt.md](TOOL_CREATOR_GUIDE.pt.md)
Consulte aqui o guia passo a passo.
...
- **Solicitações pull**: Faça um fork do repositório, aplique suas alterações e envie um PR. Consulte [DEVELOP.md](../src/uagent/docs/DEVELOP.md) para a configuração de desenvolvimento e as diretrizes.
...
Consulte [ARCHITECTURE.md](ARCHITECTURE.md) para conhecer os contratos de implementação duradouros que abrangem o ciclo de vida do A2A, os contextos I18N, a instalação de dependências opcionais, a segurança das ferramentas, as capacidades dos provedores, os limites de confiança do OAuth, os eventos estruturados e a verificação de aceitação.
