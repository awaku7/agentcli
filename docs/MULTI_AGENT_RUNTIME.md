...
`RemoteAgentRuntime` supports retry with bounded backoff, task polling via `wait()`, SSE task subscription via `subscribe()`, listing, cancellation, and remote checkpoint get/save. Remote execution remains subject to A2A authentication, ToolPolicy, Enterprise Policy, cancellation, and task-store behavior on the remote server.
