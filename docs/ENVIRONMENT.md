...
- `UAGENT_CONSECUTIVE_TOOL_CALL_LIMIT`: Maximum number of consecutive fresh tool calls across all tool names and arguments (default: `8`).
...
- `UAGENT_AUTO_REVIEW_LANGUAGE`: Language or locale requested for auto-pilot reviewer feedback (for example `ja` or `en`). Defaults to the current UI locale; `COMPLETE` and `CONTINUE` remain fixed protocol tokens.
...
- Non-interactive exit codes: `0` for completed, `1` for failed/loop-guard/max-rounds, and `130` for cancelled or interrupted runs.
